...
apiRouter.post('/compile/', async (req, res) => {
  ...
  try {
    ...
    compiler.compile().then(code => {
      //console.log(code[1]);
      const minifiedArray = `/** SGkgSlMtQ29kZXIsIEkgYWR2aXNlIHlvdSB0byB0aGluayBiZWZvcmUgeW91IHRyeSB0byBieXBhc3MgdGhlIHByb3RlY3Rpb24sIA== */\n`;
  
      let outputCode = code[0];
   
      return res.json({ code: minifiedArray + code[0] });
    })//a
