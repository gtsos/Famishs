...
const CraftTranslator_1 = require("../craft/CraftTranslator");
class Building extends Entity_1.Entity {
    ...
    onDead(damager) {
        ...
        if (damager && damager.type == EntityType_1.EntityType.PLAYERS) {
            let craftId = (0, CraftTranslator_1.findCraftByItemId)(ItemIds_1.ItemIds[this.ownerClass.itemName.toUpperCase()]);
            let craftItem = this.gameServer.crafts.find(c => craftId == (0, CraftTranslator_1.findCraftIdFromItem)((c.itemName)));
            if (craftItem) {
                let recipes = craftItem.recipe;
                for (let i = 0; i < recipes.length; i++) {
                    let item = recipes[i];
                    let itemId = ItemIds_1.ItemIds[item[0].toUpperCase()];
